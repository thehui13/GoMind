...
// OpenAIEmbedder 实现 embedding.Embedder 接口，使用 OpenAI/Gemini API
...
func (e *OpenAIEmbedder) EmbedStrings(ctx context.Context, texts []string, opts ...embedding.Option) ([][]float64, error) {
	...
	for i, data := range resp.Data {
		float64Embedding := make([]float64, len(data.Embedding))
		for j, v := range data.Embedding {
			float64Embedding[j] = float64(v)
		}
		result[i] = float64Embedding
	}
	return result, nil
}
...
func NewRAGIndexer(filename, embeddingModel string) (*RAGIndexer, error) {
	...
	var apiKey string
	if cfg.RagModelConfig.RagEmbeddingType == "openai" {
	...
	if cfg.RagModelConfig.RagEmbeddingType == "openai" {
		// 使用 OpenAI/Gemini embedding
		baseURL := "https://generativelanguage.googleapis.com/v1beta/openai/"
		embedder = NewOpenAIEmbedder(baseURL, apiKey, embeddingModel)
	} else {
		// 使用阿里云 Ark embedding
...
func NewRAGQuery(ctx context.Context, username string) (*RAGQuery, error) {
	...
	if cfg.RagModelConfig.RagEmbeddingType == "openai" {
		// 使用 OpenAI/Gemini embedding
		baseURL := "https://generativelanguage.googleapis.com/v1beta/openai/"
		embedder = NewOpenAIEmbedder(baseURL, apiKey, cfg.RagModelConfig.RagEmbeddingModel)
	} else {
		// 使用阿里云 Ark embedding
		embedConfig := &embeddingArk.EmbeddingConfig{
